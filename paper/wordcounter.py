...
def main(pdf_file:str, output_file:str='wordcount.csv', save:bool=False):
    ...
    chapters = ['Cover/Preliminaries', '1. introduction', '2. state of the art', '3. design of the solution', '4. results', '5. conclusions', 'regulatory framework', 'bibliography']
    num_words = 0
    chapter_count = {}
    pages_count = {}
    chapter_counter = 0
    for page_num in range(num_pages):
        ...
        if text[:50].lower().strip().startswith(chapters[min(chapter_counter+1,len(chapters)-1)]):
            chapter_counter += 1
    ...
    words_per_chapter = re.sub("regulatory framework", "End Matters", words_per_chapter, flags=re.IGNORECASE)
    typer.echo(f"Word,Pages per chapter:\n\t{words_per_chapter}")
    typer.echo(f"Page with most words: {max(pages_count, key=pages_count.get)+1} ({pages_count[max(pages_count, key=pages_count.get)]} words)")
    typer.echo(f"Page with least words: {min(pages_count, key=pages_count.get)+1} ({pages_count[min(pages_count, key=pages_count.get)]} words)")
