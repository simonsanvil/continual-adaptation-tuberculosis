...
%  drug discovery and early detection of
    ...
    The idea of bringing MAPE-K loops to the context of this work is that much like traditional software systems, ML Systems can also benefit from the study of SAS techniques (and the ample literature that exists on the topic) to improve their robustness and reliability. Indeed, the idea of continual adaptation is not new to the ML field, with well-known researchers arguing that it is a fundamental aspect of intelligence \cite{,thrun_lifelong_1995, parisi_continual_2019}. 
    
    However, the design of such systems presents unique challenges that must be addressed to ensure their reliability and robustness. In particular, ML models differ from traditional software systems in that they are often trained on a fixed dataset but deployed in a dynamic environment, with the assumption that the data used to train the model is representative of the data it will encounter in the real world.
    
    In practice, the assumption described above is not met, more often than not, the model's performance degrades over time as the data distribution changes. This phenomenon is known as \textit{data drift} and is one of the most common causes of degradation of a learning system \cite{huyen_designing_2022}.

    While data drift is one of the most studied phenomena in the context of continual adaptation, it is not the only one. Table \ref{tab:causes_of_ml_degradation} lists some of the most relevant causes of degradation of a machine-learning system and their characteristics. It is meant to give an overview of the different phenomena that can affect a model's performance and the kind of challenges that must be addressed in the design of a system that reliably adapts to changes in its environment.
    ...
    \begin{table}[H]
        ...
        \midrule
        % catastrophic forgetting and other learning phenomena
        \multirow{2}{*}{\small \textbf{Learning phenomena}} & \footnotesize
            \textbf{Catastrophic forgetting:} The model forgets what it has learned in the past when learning new information. & \\
            \cline{2-3} & \footnotesize
            \textbf{Catastrophic interference:} The model forgets what it has learned in the past when learning new information. & \\
