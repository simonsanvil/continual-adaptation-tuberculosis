...
%    \clearpage
    ...
    The two ideas are closely related, with \textit{continual} (or \textit{lifelong}) learning often used to refer specifically to ML systems while \textit{SAS} to any software that adapts itself to disruptive changes. The core idea of both is the same, though, the ability of a system to `survive' variations in its environment with or without human intervention and continue to perform its intended function. %In this work, we use the terms \textbf{continual adaptation}, drawing inspiration from both concepts. 
