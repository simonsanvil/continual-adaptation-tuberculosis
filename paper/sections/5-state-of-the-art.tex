...
\begin{document}
   ...
   The work in this thesis is all about adopting continual learning for healthcare applications. It is not only suitable in applications where data arrives in a stream (e.g., wearable health monitors) but also in cases where the data is collected periodically (e.g., medical imaging). In both cases, the data is likely to change over time, and the model needs to adapt to it. We'll continue this section with techniques that can be used to implement or improve a continual learning system and face the challenges described above.
    ...
    Active learning is a technique that has been used in the literature to improve the performance of ML models in medical applications. Chen et al. (2015) \cite{chen_study_2015}, for example, used active learning to improve the performance of a named entity recognition (NER) model to extract important entities from clinical texts (diseases, medications, procedures). The authors used three different data sampling strategies to select the most informative instances to label, to achieve an F-measure of 0.80, their method required 66\% fewer labeled instances than a baseline model trained on randomly selected data.

    The literature on active learning identifies three main strategies for selecting the most informative instances to label \cite{huyen_designing_2022, chen_study_2015, settlesActiveLearning2012}. These are:
    
    \begin{itemize}
        \item \textbf{Uncertainty sampling} is a strategy that selects the instances that the model is most uncertain about. This is done by selecting the instances for which the model's prediction is closest to 0.5 (i.e., the model is unsure about its prediction).
        \item \textbf{Expected model change} is a strategy that selects the instances for which the model's parameters are most likely to change. The way this is done depends on the model. For example, for a typical DNN trained with gradient descent methods, this can be done by selecting the instances for which the gradient of the loss function might be the largest. 
        \item  \textbf{Query by committee} is a strategy that selects the instances for which the model's predictions are most diverse. This is done by training multiple models on the same dataset and selecting the instances for which the ensemble disagrees the most. Thus focusing on a more model-agnostic approach to uncertainty sampling.
    \end{itemize}
    
    In the context of this project, active learning enables the development of accurate models using significantly less labeled data, paving the way for more efficient and cost-effective healthcare systems. For this reason, it is a technique that we consider to be of great importance for the development of the system proposed in this work.
    ...
    Adversarial examples are a major concern in healthcare applications. An attacker may purposely create adversarial examples to fool the model into making incorrect predictions, which can have severe consequences like misdiagnosing a patient or prescribing them unnecessary treatment. 
    ...
    \begin{figure}
        \includegraphics[width=0.61\linewidth]{figures/medical-adversarial-attacks.jpeg}
