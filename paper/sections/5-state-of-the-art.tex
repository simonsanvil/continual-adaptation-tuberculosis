...
\begin{document}
    ...
    In the second case, the one we are interested in, the problem is about using the MAPE-K loop as a means to improve the learning process of an ML model. 
    For example, to detect when the model is degrading (Monitoring) and then use a tactic to improve its performance (Execution).
